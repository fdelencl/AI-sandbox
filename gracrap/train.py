...
# m = Memory(140000)
o = optim.RMSprop(n.parameters(), lr=0.0001, momentum=0.001)
...
def optimize(outputs, frame, score, bonus_factor):
	...
	for i in range(0, len(outputs)):
		profile.append((sum([(1 - exp(-30 / ((i - f) ** 2))) * 0.5 if i != f and f < frame - 350 else 1 if f < frame - 350 else 0 for f in frames]) + 1) * exp(-100 / ((i - frame - 5) ** 2)))
	for i in range(0, len(outputs)):
...
def loop():
	...
	print(env.data.memory.blocks[0][789], _info["enemy_1.type"])
