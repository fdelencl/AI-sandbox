...
class Model(nn.Module):
	...
	def forward(self, input, hidden):
		...
		ene = F.relu(self.enemy_linear1(ene))
		ene = F.relu(self.enemy_linear2(ene))
		ene = F.relu(self.enemy_linear3(ene))
		ene = F.relu(self.enemy_linear4(ene))
		ene = F.relu(self.enemy_linear5(ene))
		ene = F.relu(self.enemy_linear6(ene))
		ene = F.relu(self.enemy_linear7(ene))
		ene = F.relu(self.enemy_linear8(ene))
		ene = F.relu(self.enemy_linear9(ene))
		ene = ene.view(ene.size(0), 11, 3)

		hx, cx = self.lstm(x, (hx, cx))
		pos = self.position_linear(hx)
		return (pos, ene), (hx, cx), features
	...
	def estimate(self, screen, hidden):
		...
		out, hidden, features = self(input, hidden)

		return out, hidden, features
